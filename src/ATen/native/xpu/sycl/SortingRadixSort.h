...
#pragma unroll
  ...
  inline void process_partial_tile(int group_offset, int group_end) {
    // Use a relative loop variable to avoid int32 overflow: when group_offset is
    // near INT_MAX, `group_offset + lid_ + k*GROUP_THREADS` wraps to INT_MIN on
    // the next iteration, making the loop condition false-positive.
